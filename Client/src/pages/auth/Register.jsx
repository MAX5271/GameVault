...
function Register() {
  ...
  const passwordChecks = [
    { label: "8-24 characters", test: password.length >= 8 && password.length <= 24 },
    { label: "An uppercase letter", test: /[A-Z]/.test(password) },
    { label: "A lowercase letter", test: /[a-z]/.test(password) },
    { label: "A number", test: /[0-9]/.test(password) },
    { label: "A special character (!@#$%)", test: /[!@#$%]/.test(password) },
  ];
          ...
          {success ? (
            ...
            <>
              ...
              <div className={styles.formGroup}>
                ...
                <ul
                  className={pwdFocus && !validPass ? styles.checklist : styles.offscreen}
                  aria-live="polite"
                >
                  {passwordChecks.map(({ label, test }) => (
                    <li key={label} className={test ? styles.valid : styles.invalid}>
                      {test ? "✓" : "✗"} {label}
                    </li>
                  ))}
                </ul>
